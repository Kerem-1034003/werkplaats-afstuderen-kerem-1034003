...
def translate_and_rewrite_name(category_name, target_language="nl"):
    ...
    # Prompt die een correcte vertaling in 1 woord afdwingt, vooral voor Engels of Duits
    prompt = f"""
    ...
    Geef enkel het vertaalde of gecorrigeerde woord zonder extra uitleg.
    """
...
def generate_category_description(category_name):
    prompt = f"""
    Schrijf een uitgebreide en informatieve beschrijving van ongeveer 500 woorden voor de productcategorie '{category_name}'.
...
# Functie om `meta:_yoast_data` op te bouwen met bedrijfsnaam-aanvulling
def build_meta_yoast_data(focus_keyword):
    # Genereer de meta title
    meta_title_prompt = f"""
    Maak een aantrekkelijke en SEO-geoptimaliseerde meta title van maximaal 60 karakters beginnend met '{focus_keyword}'.
    """
    ...
    """
    meta_desc_response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
    ...
    yoast_data = f"""a:6:{{
        s:21:"wpseo_keywordsynonyms";s:4:"[""]";
        s:11:"wpseo_title";s:{len(meta_title)}:"{meta_title}";
        s:10:"wpseo_desc";s:{len(meta_description)}:"{meta_description}";
        s:13:"wpseo_focuskw";s:{len(focus_keyword)}:"{focus_keyword}";
        s:13:"wpseo_linkdex";s:2:"44";
        s:19:"wpseo_content_score";s:1:"0";
    }}"""
    return yoast_data
...
df.to_excel('herschreven_excel/simpledeal/simpledealcat1.0.xlsx', index=False)
